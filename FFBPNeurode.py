...
def main():
    ...
    try:
        ...
        print(
            "Error - Is sigmoid_derivative named correctly, created in BPNeurode and decorated as a static method?")
    ...
    try:
        ...
        if hnodes[0].register_back_input(onodes[1]) and not hnodes[
            1].register_back_input(onodes[1]):
            print("Pass")
        else:
            print("Error - register_back_input is not responding correctly")
    except:
        print(
            "Error - register_back_input is raising an error.  Is it named correctly?  Check your syntax")
    ...
    try:
        ...
        if .0479 < onodes[0].delta < .0481:
            ...
            print(
                "Error - calculate delta is not returning the correct value.  Check the math.")
            print(
                "        Hint: do you have a separate process for hidden nodes vs output nodes?")
    except:
        print(
    ...
    try:
        ...
        if onodes[0].learning_rate == .05:
            if .4 + .06 * onodes[0].learning_rate - .001 < \
                    onodes[0].input_nodes[hnodes[0]] \
                    < .4 + .06 * onodes[0].learning_rate + .001:
                print("Pass")
            else:
                print(
                    "Error - weights not updated correctly.  If all other methods passed, check update_weights")
        else:
            print("Error - Learning rate should be .05, please verify")
    except:
        print(
            "Error - update_weights is raising an error.  Is it named correctly?  Check your syntax")
    print(
    ...
    if (value1 - value1a > 0) and (value2a - value2 > 0):
        ...
        print(
            "Fail - the network did not make progress.  If everything else passed, first try these hints:")
        print("    Hint1: Check receive_back_input()")
        print(
            "    Hint2: Check back_fire(), especially the order of method calls")
        print(
            "If you hit a wall, be sure to seek help in the discussion form, from the instructor and from the tutors")
